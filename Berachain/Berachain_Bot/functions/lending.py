...
def check_and_approve_honey(amount):
    ...
    try:
        ...
        if balance < amount:
            print(f"Insufficient HONEY balance. Have {w3.from_wei(balance, 'ether')}, need {w3.from_wei(amount, 'ether')}")
            return False
...
def supply_honey(amount_in_honey=1):
    ...
    try:
        ...
        if not check_and_approve_honey(amount_in_wei):
            return False
        ...
        transaction = contract.functions.supply(
            ...
            address,         # onBehalfOf (your own address)
            18              # referralCode
        ).build_transaction({
            'from': address,
            'nonce': nonce,
...
def supply_honey_raw(amount_in_honey=1):
    try:
        # Convert amount to Wei (18 decimals)
        amount_in_wei = w3.to_wei(amount_in_honey, 'ether')
